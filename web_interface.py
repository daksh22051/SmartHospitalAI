...
class EnvironmentManager:
    ...
    def execute_step(self, action: int) -> Tuple[bool, Dict[str, Any]]:
        ...
        try:
            ...
            print(f"[STEP] {json.dumps({'step': self.episode_count, 'action': action, 'action_name': action_names.get(action, 'UNKNOWN'), 'rationale': 'api-call', 'reward': {'value': float(reward), 'normalized_0_to_1': reward_normalized}, 'done': done}, separators=(',', ':'))}", flush=True)
            
            if done:
                final = self.env.state().get("readable", {}) if self.env else {}
                avg_reward = self._cumulative_reward / max(self.episode_count, 1)
                score_normalized = float(0.5 * (np.tanh(float(avg_reward) / 40.0) + 1.0))
                end_result = {
                    "task": self.current_task or "medium",
                    "seed": self.current_seed,
                    "steps": self.episode_count,
                ...
                print(f"[END] {json.dumps(end_result, separators=(',', ':'))}", flush=True)
                print(f"INFERENCE_RESULT={json.dumps(end_result, separators=(',', ':'))}", flush=True)
...
def reset():
    """OpenEnv required /reset endpoint - wraps /api/reset"""
    return reset_environment()

@app.route('/step', methods=['POST'])
def step():
    """OpenEnv required /step endpoint - wraps /api/step"""
    return execute_step()
...
if __name__ == '__main__':
    ...
    print("[STARTUP] Smart Hospital Orchestration API starting...", flush=True)
    port = int(os.getenv('PORT', '7860'))
    print(f"[STARTUP] Available endpoints: /reset, /step, /api/reset, /api/step, /api/health, /api/status", flush=True)
    print(f"[STARTUP] Open your browser: http://localhost:{port}", flush=True)
